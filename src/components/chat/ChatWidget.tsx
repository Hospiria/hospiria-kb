...
import {
  MessageCircle, X, Plus, Send, Loader2, History, FileText, Trash2, ArrowLeft, Sparkles,
} from 'lucide-react'
  ...
  async function openConversation(id: string) {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        setMessages(
          (data.messages ?? []).map((m: { role: 'user' | 'assistant'; content: string; sources?: SopSource[] }) => ({
            role: m.role,
            content: m.content,
  ...
  return (
    <>
      ...
      {!open && (
        <button
          onClick={openWidget}
          aria-label="Open knowledge base assistant"
        ...
        <div className="fixed bottom-6 right-6 z-50 w-[400px] max-w-[calc(100vw-2rem)] h-[600px] max-h-[calc(100vh-3rem)] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-navy-900 text-white flex-shrink-0">
            <div className="flex items-center gap-2">
              {view === 'history' && (
                <button onClick={() => setView('chat')} className="text-white/70 hover:text-white" aria-label="Back">
              ...
              <div className="leading-tight">
                <p className="text-sm font-semibold">KB Assistant</p>
                <p className="text-[10px] text-white/55">Ask about any SOP</p>
              </div>
            </div>
          ...
          {view === 'history' ? (
            <div className="flex-1 overflow-y-auto p-2">
              {conversations.length === 0 ? (
                <p className="text-center text-sm text-gray-400 py-10">No previous chats yet.</p>
              ) : (
                conversations.map(c => (
                  <button
                    key={c.id}
                    onClick={() => openConversation(c.id)}
                    className="w-full text-left group flex items-center gap-2 px-3 py-2.5 rounded-lg hover:bg-gray-50 transition-colors"
                  >
              ...
              <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-4 bg-slate-50">
                {messages.length === 0 && !sending && (
                  <div className="text-center py-8 px-4">
                    <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-teal-400 to-teal-600 flex items-center justify-center mx-auto mb-3">
                    ...
                    <div className="space-y-1.5 text-left">
                      {[
                        ...
                        <button
                          key={s}
                          onClick={() => setInput(s)}
                          className="block w-full text-left text-xs text-navy-600 bg-white border border-gray-200 rounded-lg px-3 py-2 hover:border-teal-300 hover:bg-teal-50/40 transition-colors"
                        >
                          {s}
                        </button>
                      ))}
                ...
                {sending && (
                  <div className="flex justify-start">
                    <div className="bg-white border border-gray-200 rounded-2xl rounded-bl-sm px-3.5 py-2.5 flex items-center gap-2 text-sm text-gray-400">
              ...
              <div className="border-t border-gray-200 p-3 flex-shrink-0 bg-white">
                <div className="flex items-end gap-2">
                  <textarea
                    ...
                    onKeyDown={onKeyDown}
                    rows={1}
                    placeholder="Ask about a SOP…"
                    className="flex-1 resize-none max-h-28 text-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                  />
                  <button
                    onClick={send}
                    disabled={!input.trim() || sending}
                    className="w-10 h-10 flex-shrink-0 rounded-xl bg-navy-700 text-white flex items-center justify-center hover:bg-navy-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                    aria-label="Send"
                  >
